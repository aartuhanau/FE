...
const HIDDEN_SET = new Set(JSON.parse(localStorage.getItem(LIST_ITEM_TO_SET.HIDDEN) ?? '[]'))
const FAVORITES_SET = new Set(JSON.parse(localStorage.getItem(LIST_ITEM_TO_SET.FAVORITES) ?? '[]'))
const COMPARE_SET = new Set(JSON.parse(localStorage.getItem(LIST_ITEM_TO_SET.COMPARE) ?? '[]'))
...
const ACTION_SET = new Set([CLASSES.MENU_ALL, CLASSES.MENU_COMPARE, CLASSES.MENU_FAVORITES])
...
function activeButton(listItem, action) {
    ...
    if (action.classList.contains(CLASSES.ACTION_HIDDEN) && !isHiddenListItemVisible) {
        action.closest(`.${CLASSES.LIST_ITEM}`).classList.add(CLASSES.LIST_ITEM_HIDDEN)
    }
}
...
function saveSets() {
    localStorage.setItem(LIST_ITEM_TO_SET.HIDDEN, JSON.stringify([...HIDDEN_SET]))
    localStorage.setItem(LIST_ITEM_TO_SET.FAVORITES, JSON.stringify([...FAVORITES_SET]))
    localStorage.setItem(LIST_ITEM_TO_SET.COMPARE, JSON.stringify([...COMPARE_SET]))
}

function initializeMenu() {
    document.querySelector(`.${CLASSES.MENU_FAVORITES}`).addEventListener("click", handlers[CLASSES.MENU_FAVORITES])
    document.querySelector(`.${CLASSES.MENU_COMPARE}`).addEventListener("click", handlers[CLASSES.MENU_COMPARE])
    document.querySelector(`.${CLASSES.MENU_ALL}`).addEventListener("click", handlers[CLASSES.MENU_ALL])
    document.querySelector(`.${CLASSES.MENU_HIDE}`).addEventListener("click", handlers[CLASSES.MENU_HIDE])
}

const handlers = {
    [CLASSES.MENU_FAVORITES]: () => handleMenuItemClick(CLASSES.MENU_FAVORITES, el => !FAVORITES_SET.has(el.getAttribute(ID))),
    [CLASSES.MENU_COMPARE]: () => handleMenuItemClick(CLASSES.MENU_COMPARE, el => !COMPARE_SET.has(el.getAttribute(ID))),
...
function handleChooser() {
    ...
    let element = document.querySelector(`.${CLASSES.MENU_ITEM_ACTIVE}.${CLASSES.MENU_ITEM}`) ?? document.querySelector(`.${CLASSES.MENU_ALL}`)

    let activeAction = [...element.classList].filter(className => ACTION_SET.has(className))
...
function changeListItemVisibility(predicate, element) {
    if (predicate(element) || (!isHiddenListItemVisible && HIDDEN_SET.has(element.getAttribute(ID)))) {
...
function initializeListItems() {
    document.querySelectorAll(`.${CLASSES.LIST_ITEM}`).forEach(listItem => updateListItem(listItem))
}
